...
> This example Analysis demonstrates the use of linear regression. Specifically, we will examine which variables best predict GPA. First, we will fit a model predicting GPA by high school grades. Then, we will use a model that predicts GPA by SAT scores. Finally, we wil fit a model that uses both high shool grades and SAT scores to predict GPA.

# Data Import
As the data is given in form of a `.csv` file we have to import it using the `read.csv()` or `readr::read_csv()` functions.
